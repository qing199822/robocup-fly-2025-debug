...
`src/ego_fusion_search/local_mapping`、`src/ego_fusion_search/search_msgs`、`src/ego_fusion_search/ego_adapter` 和 `src/ego_fusion_search/search_coordinator` 是队伍自有源码。`local_mapping` 复用系统安装的 ROS、PCL 和 OpenCV，并依赖上表已登记的 `darknet_ros_msgs`；`search_msgs` 只使用 ROS 消息与服务生成；`ego_adapter` 通过已安装的 ROS 消息接口读取外部 EGO 输出；`search_coordinator` 只负责队伍的局部目标和任务代次。四个包都没有复制 EGO 或其他新的第三方源码进入仓库。
